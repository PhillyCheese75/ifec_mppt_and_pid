...
static adcListComponent_t mppt_one_voltage = {
                                              ...
                                              V_PV_SENSE_R1, V_PV_SENSE_R2, NONE
                                             };
...
static adcListComponent_t buck_5V_voltage  = {
                                              ADCD_BASE, ADCDRESULT_BASE,
                                              0, 0, 0.0, 0.0, 0.0,
                                              ADC_SOC_NUMBER0, Voltage_Component,
                                              BUCK_5V_OUTPUT_R1, BUCK_5V_OUTPUT_R2, NONE
                                             };

static adcListComponent_t buck_3V3_voltage = {
                                              ADCD_BASE, ADCDRESULT_BASE,
                                              0, 0, 0.0, 0.0, 0.0,
                                              ADC_SOC_NUMBER1, Voltage_Component,
                                              BUCK_3V3_OUTPUT_R1, BUCK_3V3_OUTPUT_R2, NONE
                                             };

static adcListComponent_t battery_voltage = {
                                             ADCD_BASE, ADCDRESULT_BASE,
                                             0, 0, 0.0, 0.0, 0.0,
                                             ADC_SOC_NUMBER3, Voltage_Component,
                                             V_BATT_SENSE_R1, V_BATT_SENSE_R2, NONE
                                            };

static adcListComponent_t battery_current = {
                                             ADCD_BASE, ADCDRESULT_BASE,
                                             0, 0, 0.0, 0.0, 0.0,
                                             ADC_SOC_NUMBER2, Current_Component,
                                             0, 0, BATI
                                            };
...
//    ADC_setVREF(ADCB_BASE, ADC_REFERENCE_INTERNAL, ADC_REFERENCE_3_3V);
    ...
    ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER0, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN0, 30); //VBOT - 1
    ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER1, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN1, 30); //IBOT - 1
    ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER3, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN3, 12); //ITOP - 2
    //ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER2, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN4, 13); //VTOP - 2 ???? OLD PIN 24
    ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER2, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN2, 13); //NEW VTOP- 2 PIN 18



    // Configure SOCs of ADCD
    ADC_setupSOC(ADCD_BASE, ADC_SOC_NUMBER0, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN3, 13); //5V
    ADC_setupSOC(ADCD_BASE, ADC_SOC_NUMBER1, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN2, 12); //3.3V
    ADC_setupSOC(ADCD_BASE, ADC_SOC_NUMBER2, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN1, 3); //IBAT
    //ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER4, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN5, 13); //VBAT - ????
    ADC_setupSOC(ADCD_BASE, ADC_SOC_NUMBER3, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN0, 14);//VBAT NEW PIN 28
...
void update_conversion(adcListComponent_t * adcComponent) {
    ...
    if(adcComponent->component_type == Voltage_Component) {
        ...
        if(adcComponent->Hall == MPPT_ONE) {
            adcComponent->current = I_SENSED1(adcComponent->stepped_down_volts);
        }
        if(adcComponent->Hall == MPPT_TWO) {
            adcComponent->current = I_SENSED2(adcComponent->stepped_down_volts);
        }
        else if(adcComponent->Hall == BATI) {
            adcComponent->current = I_SENSEDB(adcComponent->stepped_down_volts);
        }
    }
}
